...
def walk(lines, pos, d, obstacle, d_loop):
    ...
    new_pos = (r+x, c+y)
    if is_inside(lines, new_pos) and (lines[new_pos[0]][new_pos[1]] == '#' or new_pos == obstacle):
...
print( "p1: ", evaluate_path(lines, start, d, None, loop)[0])
 
p2 = 0
for row in range(ROWS):
    for col in range(COLS):
        reset_loop(loop, lines[start[0]][start[1]])
        
        size, in_loop = evaluate_path(lines, start, d, (row, col), loop)
